...
export function ColorControl({ value, onChange, fallback = '#373434' }) {
  const id = useId();
  const [draft, setDraft] = useState(value || '');
  const invalid = Boolean(draft && !normalizeColor(draft));
  return <fieldset className="color-control"><legend>Color</legend>
    ...
    <p id={id}>{invalid ? 'Enter a hex color, e.g. #86c5ff.' : 'Shadow, hover and text colors adapt automatically.'}</p>
  </fieldset>;
}
