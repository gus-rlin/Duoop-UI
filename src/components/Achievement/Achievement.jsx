...
export function Achievement({ item, presentation = 'card', visual = 'medallion', progressDisplay = 'bar', size = 'md', onDetails, onClaim, claimStatus = 'idle', loading = false, error, onRetry, locale = 'en', className = '' }) {
  ...
  return <Card ref={card} variant={presentation === 'badge' || presentation === 'row' ? 'outline' : 'elevated'} size={size} className={`duoop-achievement ${className}`} data-state={item.state} data-presentation={presentation} data-size={size} data-concealed={concealed} data-visual={visual} aria-label={title} loading={loading}>
